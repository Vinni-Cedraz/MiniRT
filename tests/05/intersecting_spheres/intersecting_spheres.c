...
#define scenario1 CYAN "\nGiven r ← ray(point(0, 0, -5), vector(0, 0, 1))\n" \
						...
						"Then inters.count = 2\n" \
						"And inters.arr->t = 4.0\n" \
						"And inters.arr->next->t = 6.0\n" RESET
...
Test(intersecting_rays, intersects_at_two_points, .description = scenario1) {
	...
	cr_expect_eq(TRUE, doubles_eq(inter.arr[0].t, 4.0));
	cr_expect_eq(TRUE, doubles_eq(inter.arr[1].t, 6.0));
}
...
Test(intersecting_rays, tangent_intersection, .description = scenario2) {
	...
	cr_expect_eq(inter.count, 2);
	cr_expect_eq(TRUE, doubles_eq(inter.arr[0].t, 5.0));
	cr_expect_eq(TRUE, doubles_eq(inter.arr[1].t, 5.0));
}
...
Test(intersecting_rays, ray_starts_inside_of_a_sphere, .description = scenario4) {
	...
	cr_expect_eq(inter.count, 2);
	cr_expect_eq(TRUE, doubles_eq(inter.arr[0].t, -1.0));
	cr_expect_eq(TRUE, doubles_eq(inter.arr[1].t, 1.0));
}
...
Test(intersecting_rays, sphere_is_behind_the_ray, .description = scenario5) {
	...
	cr_expect_eq(inter.count, 2);
	if (doubles_eq(inter.arr[0].t, -6.0) == FALSE)
		printf(RED"inter.arr->t %f\n", inter.arr[0].t);
	if (doubles_eq(inter.arr[1].t, -4.0) == FALSE)
		printf(RED"inter.arr[0].t %f\n"RESET, inter.arr[1].t);
	cr_expect_eq(TRUE, doubles_eq(inter.arr[0].t, -6.0));
	cr_expect_eq(TRUE, doubles_eq(inter.arr[1].t, -4.0));
}
...
#define scenario6 CYAN \
		...
		"Then i.count = 2\n"               \
		"And i.arr[0].t = sqrt(3)\n" \
		"And i.arr[1].t = sqrt(3)\n"RESET
...
Test(intersecting_rays, the_ray_is_diagonal_and_tangencies_the_sphere, .description = scenario6) {
	...
	if (low_precision_doubles_eq(i.arr[0].t, sqrt(3)) == FALSE)
		printf(RED"inter.arr[0].t %f\n", i.arr[0].t);
	if (low_precision_doubles_eq(i.arr[1].t, sqrt(3)) == FALSE)
		printf(RED"inter.arr[1].t %f\n"RESET, i.arr[1].t);
	cr_expect_eq(TRUE, low_precision_doubles_eq(i.arr[0].t, sqrt(3)));
	cr_expect_eq(TRUE, low_precision_doubles_eq(i.arr[1].t, sqrt(3)));
}
