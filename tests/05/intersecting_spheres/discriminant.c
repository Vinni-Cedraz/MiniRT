...
Test(discriminant, negative_discriminant_no_intersections, .description = scenario1) {
	...
	if (_discriminant(distance_sphere_to_ray, &ray, &(t_bhaskara){0}) >= 0)
		printf(RED"discriminant: %f\n"RESET, _discriminant(distance_sphere_to_ray, &ray, &(t_bhaskara){0}));
	cr_expect_eq(_discriminant(distance_sphere_to_ray, &ray, &(t_bhaskara){0}) < 0, TRUE);
}
