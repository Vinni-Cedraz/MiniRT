...
Test(suite_name, magnitude_of_normalized_vector_1_2_3) {
    normalize_vector(vecs[1], res);
    cr_expect_eq(compare_floats(vector_magnitude(res), 1), TRUE);
}

Test(suite_name, normalization_of_zero_magnitude_vector_should_fail) {
	cr_expect_eq(normalize_vector((t_tuple){0, 0, 0, VECTOR}, res), -1);
}

Test(suite_name, normalization_of_point_should_fail) {
	cr_expect_eq(normalize_vector((t_tuple){1, 2, 3, POINT}, res), -1);
}
