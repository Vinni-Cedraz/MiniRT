...
t_2x2matrix				*_3x3submatrix( t_3x3matrix m, int row_to_delete,
							int col_to_delete);
t_bool					tuples_neq( double *result,  double *expected,
							int len);
t_bool					_3x3matrices_eq(t_3x3matrix a, t_3x3matrix b);
t_bool					_2x2matrices_eq(t_2x2matrix a, t_2x2matrix b);
t_3x3matrix				*_4x4submatrix( t_matrix m, int not_row,
							int not_col);
double					_3x3minor( t_3x3matrix m, int row, int col);
double					_3x3cofactor( t_3x3matrix m, int row, int col);
double					_3x3determinant( t_3x3matrix m);
double					_4x4determinant( t_matrix m);
double					_3x3minor( t_3x3matrix m, int row, int col);
double					_4x4minor( t_matrix m, int row, int col);
double					_3x3cofactor( t_3x3matrix m, int row, int col);
...
t_matrix				*create_scaling_matrix( double x,  double y,
							 double z);
...
void					sphere_normal_at( t_shape *sphere,  t_tuple p,
							t_tuple res);
void					plane_normal_at( t_shape *sphere,  t_tuple p,
							t_tuple res);
void					cylinder_normal_at( t_shape *cyl,  t_tuple p,
							t_tuple res);
void					reflect(t_tuple vector, t_tuple normal,
							t_tuple _return);
t_material				create_material(void);
void					calculate_lighting(t_lighting *obj, t_tuple result);
t_r				make_aslib_test(void);
...
void					intersect_caps( t_cylinder *cyl,  t_ray r,
							t_node **head);
...
static inline void print_tuple( t_tuple a) { printf("X: %f, Y: %f, Z: %f, W: %f\n", a[X], a[Y], a[Z], a[W]); }

static inline void print_4x4matrix( t_matrix a) {
    print_tuple(a.row_1);
    print_tuple(a.row_2);
    print_tuple(a.row_3);
    print_tuple(a.row_4);
