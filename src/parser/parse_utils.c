...
// if the count of commas is not 2, return ERROR, if the type is VECTOR and the
// tuple is not normalized, return ERROR, if the type is COLOR and the tuple is
// not between 0 and 255, return ERROR
static int	validate_tuple(t_tuple tuple, int commas, short type)
{
	if (commas != 2)
	{
