...
void	sphere_normal_at( \
		t_shape *sphere, t_tuple wrld_p, t_tuple normal)
{
	t_tuple	obj_point;
	t_tuple	obj_nrml;
	t_tuple	wlrd_nrml;
...
void	plane_normal_at( \
		t_shape *plane, t_tuple wrld_p, t_tuple res)
{
	t_tuple	obj_nrml;
	t_tuple	wlrd_nrml;
...
void	cylinder_normal_at( \
		t_shape *cyl, t_tuple wrld_p, t_tuple res)
{
	t_tuple		obj_point;
	t_tuple		obj_nrml;
	t_tuple		wlrd_nrml;
	...
	if (dist < 1 && doubles_eq(obj_point[Y], cyl->max))
		init_tuple((t_tuple){0, 1, 0, VECTOR}, obj_nrml);
	else if (dist < 1 && doubles_eq(obj_point[Y], cyl->min))
		init_tuple((t_tuple){0, -1, 0, VECTOR}, obj_nrml);
	else
